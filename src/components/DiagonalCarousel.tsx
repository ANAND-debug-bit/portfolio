  ...
  return (
    ...
    >
      <div className={cn('absolute inset-0 overflow-hidden', viewportClassName)}>
        <motion.div
          className="absolute left-1/2 flex w-fit"
          style={{ top: '50%', marginTop: -(safeSlideSize / 2 + 18) }}
      ...
      {showControls && (
        <div
          className={cn(
            'absolute inset-x-4 bottom-5 z-10 mx-auto flex w-fit items-center justify-center gap-3 rounded-full border border-stroke bg-surface/70 px-2 text-text-primary shadow-sm backdrop-blur-md',
            controlsClassName,
          )}
        >
          <button
            ...
            className="inline-flex size-9 items-center justify-center rounded-full transition-colors hover:bg-stroke/60 disabled:cursor-not-allowed disabled:opacity-35"
